...
Please, make sure you have `R version 4.5.1 (2025-06-13)` installed in your laptop.
...
Direct estimators can be produced using the `sae` package. The `direct()` function computes the Horvitz-Thompson estimator of domain means using this formula:
...
We will calculate the direct estimators under different assumptions, allowing for four possible combinations:

-   Option A: accounts for sample weights and domain sizes, and sampling is performed without replacement. In this case, the direct estimator is calculated as follows:
...
-   Option B: accounts for domain sizes, but without sample weights nor replacement. In this case, we do not need weights, nor domain populations, just the sample size.
...
-   Option C: accounts for sample weights, domain sizes, and with replacement. In this case, the direct estimator is the same as in Option 1, since it also accounts for weights and uses domain sizes.
...
-   Option D: sampling is performed with replacement, but no sample weights nor domain sizes are considered. We calculate the estimator in the same way we did with Option B.
...
The option of choice will depend on data availability. If both domain sizes and weights are available, and we know that the survey took place without sample replacement, we can use Option A. If either the population sizes or the sample weights are missing, we will have to choose between options B, C, and D.

Let's calculate the direct estimator considering sample weights, domain sizes, and no replacement.

```{r}
# Clean and match sample population sizes
smp_area_size_df <- data.frame(
...
```{r}
# # Option B -- No replacement, with domain sizes, no weights
...
# )
# 
# # Option C -- With replacement, weights and domain sizes
...
# )
# 
# # Option D -- With replacement, no weights nor domain sizes
...
```{r, message=FALSE}
# dir_est_comparison <- data.frame(
#   Domain = dir_est_a$Domain,
#   est_A = dir_est_a$Direct,
#   est_B = dir_est_b$Direct,
#   est_C = dir_est_c$Direct,
#   est_D = dir_est_d$Direct,
#   
#   var_A = dir_est_a$SD^2,
#   var_B = dir_est_b$SD^2,
#   var_C = dir_est_c$SD^2,
#   var_D = dir_est_d$SD^2
# )
# 
# # Plot comparison
# ggpairs(
#   data = dir_est_comparison[,-1]
# ) +
#   theme_ipsum()
```
...
```{r}
# We can also compare the standard deviations
area_level_summer_est_sd <- data.frame(
  Domain = area_level_intercept$domain,
